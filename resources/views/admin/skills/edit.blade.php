    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.skills.update", [$skill->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" type="text" name="name" id="name" value="{{ old('name', $skill->name) }}">
                @if($errors->has('name'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('applicant') ? 'is-invalid' : '' }}" name="applicant_id" id="applicant_id">
                    @foreach($applicants as $id => $applicant)
                        <option value="{{ $id }}" {{ ($skill->applicant ? $skill->applicant->id : old('applicant_id')) == $id ? 'selected' : '' }}>{{ $applicant }}</option>
                    @endforeach
                </select>
